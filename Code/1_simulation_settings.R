...
#KIRC: high-medium censoring (58% do not have either event)
#LIHC: medium censoring (38% do not have either event)
...
add_risk_table <- function(
  ...
  t.risk <- map_dfr(x_breaks_filtered, function(x) {
    ...
    tibble(
      time = x,
      risk_prog = nrow(filter(d, t1 > x)) / denom,

      risk_death = nrow(filter(d, t2 > x)) / denom,

      cum_prog = nrow(filter(d, t1 <= x & event1 == 1)) / denom,

      cum_death = nrow(filter(d, t2 <= x & event2 == 1)) / denom
    )
  })
  ...
  p / p.risk + plot_layout(heights = c(3, 1))
}
