...
#Input: ./coverage_data_XXX.csv, csv files containing the coverage probabilities for each setting
...
coverage_data <- list.files(
  ...
  mutate(
    ...
    beta = str_extract(file, "(?<=_b)-?\\d+(?:\\.\\d+)?(?=\\.csv$)")
  ) %>%
...
coverage_groups <- coverage_data %>%
  ...
  pivot_longer(
    cols = c(
      ...
      "MSMI-Cox-Goodman",
      "MSMI-Cox-trWald",
      "MSMI-KM-Boot-Goodman",
      "MSMI-KM-Boot-trWald",
      "MSMI-Cox-Boot-Goodman",
      "MSMI-Cox-Boot-trWald"
    ),
    names_to = "method",
...
for (i in seq_along(group_data)) {
  dat <- group_data[[i]] %>% filter(str_detect(method, "Boot") | method == "AJ")
